...
class graph {
	public:
		...
		int shortest_path(int start, int row_len, vector <int> &path)
		{
...
class solution {
	public:
		...
		int shortest_path(int start, int row_len, vector <int> &path)
		{
			return g->shortest_path(start, row_len, path);
		}
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < matrix.size(); i++) {
		if (matrix[i][0] != 0) {
			vector <int> path;
			int ret = s.shortest_path(i * matrix[0].size(), matrix[0].size(), path);
			cout << "start from " << i * matrix[0].size() << "shortest path " << ret << endl;
			for (auto &i : path)
				cout << "(" << i / matrix[0].size() << ", " << i % matrix[0].size() << ") " << endl;
			shortest_path = min(ret, shortest_path);
		}
	}
